...
IS_LOCAL=0
if [ -f "/data/home/huangzixuan/start_vllm_minimal.sh" ]; then
  IS_LOCAL=1
fi
...
case "$ACTION" in
  start)
    if [ "$IS_LOCAL" -eq 1 ]; then
      echo "Starting minimal VRAM vLLM locally on GPU1..."
      bash /data/home/huangzixuan/start_vllm_minimal.sh
      echo "Waiting for endpoint http://127.0.0.1:8000/v1/models..."
      for i in $(seq 1 30); do
        if curl -s "http://127.0.0.1:8000/v1/models" > /dev/null; then
          echo "vLLM is READY on http://127.0.0.1:8000/v1"
          exit 0
        fi
        sleep 2
      done
  ...
  stop)
    if [ "$IS_LOCAL" -eq 1 ]; then
      echo "Stopping vLLM locally on GPU1 to free 100% GPU memory..."
      bash /data/home/huangzixuan/stop_vllm.sh
      echo "Done."
    else
  ...
  status)
    if [ "$IS_LOCAL" -eq 1 ]; then
      echo "=== GPU Status on Local GPU1 ==="
      nvidia-smi --query-gpu=memory.used,memory.total,utilization.gpu --format=csv,noheader
      echo "=== vLLM Endpoint Health (port 8000) ==="
      curl -s --connect-timeout 2 "http://127.0.0.1:8000/v1/models" || echo "vLLM endpoint inactive"
      echo ""
    else
  ...
  download-status)
    if [ "$IS_LOCAL" -eq 1 ]; then
      echo "=== Qwen3.5-4B Download Status on Local GPU1 ==="
