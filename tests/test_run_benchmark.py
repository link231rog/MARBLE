...
def test_controller_creation_and_checkpoints(tmp_path, monkeypatch):
...
def test_controller_ablations(monkeypatch):
    seen = {}
    monkeypatch.delenv("MARBLE_QWEN_API_BASE", raising=False)
...
def test_qwen_endpoint_mode(monkeypatch):
    ...
    # Fast-fail on local adapter directory conflict
...
def test_task_config_generation():
    ...
    # Single agent
    cfg_single = task_config(t, "single_agent")
    assert len(cfg_single["agents"]) == 1
    assert cfg_single["relationships"] == []

    # Environment override
    assert not str(t.environment.get("type", "")).strip()
    assert gov["environment"]["type"] == "Coding"


def test_providers_and_model_overrides(monkeypatch, tmp_path):
    for var in (
        "MARBLE_WORKER_MODEL", "MARBLE_EVAL_MODEL", "MARBLE_ZAI_MODEL",
        "MARBLE_ZAI_EVAL_MODEL", "ZAI_MODEL", "ZAI_EVAL_MODEL", "ZAI_API_KEY", "OPENAI_API_KEY",
        "MARBLE_EMPERO_MODEL", "MARBLE_EMPERO_EVAL_MODEL", "EMPERO_API_KEY",
    ):
        monkeypatch.delenv(var, raising=False)
    t = _task()
    ...
    # Empero provider
    cfg_empero = task_config(t, "no_memory", provider="empero")
    assert provider_config("empero")["base_url"] == "https://free.empero.org/v1"
    assert cfg_empero["llm"] == "openai/glm-5.3-flash"

    # CLI propagation
    from marble.experiments import run_benchmark
    monkeypatch.setattr(run_benchmark, "load_tasks", lambda *args, **kwargs: [t])
    captured = []
    monkeypatch.setattr(
        run_benchmark,
        "run_task",
...
def test_task_skip_and_resume_behavior(tmp_path, monkeypatch):
    t = _task()
    tdir = tmp_path / "heuristic" / "coding" / str(t.task_id)
    ...
    monkeypatch.setattr(
        "marble.experiments.run_benchmark._run_real_episode",
        lambda *args, **kwargs: called.append(True) or {"score_status": "available", "task_score": 1.0},
    )
    summary_mismatch = run_task(t, "heuristic", tmp_path, seed=42, max_cards=5)
    assert len(called) == 1
    assert summary_mismatch["retrieval"]["max_cards"] == 5


def test_run_roots_and_multi_run(tmp_path, monkeypatch):
    from marble.experiments import run_benchmark
    ...
    # Multi run keeps every method on disk
    tasks = [_task()]
    for baseline, task in plan_runs(["multi"], tasks):
        run_task(task, baseline, tmp_path / "multi_test", dry_run=True)
    found = sorted(
        (json.loads(p.read_text())["method"], p.parent)
        for p in (tmp_path / "multi_test").rglob("summary.json")
    )
    assert [m for m, _ in found] == sorted(BASELINES)
...
def test_real_run_without_any_key_records_error(tmp_path, monkeypatch):
    ...
    assert not (tdir / "memory_trace.jsonl").exists()
...
def test_cli_and_manifest_parameters(monkeypatch, tmp_path):
    from marble.experiments import run_benchmark
    t = _task()
    ...
    assert cfg["memory"]["max_cards"] == 0

    # CLI parameters propagate
    monkeypatch.setattr(run_benchmark, "load_tasks", lambda *args, **kwargs: [t])
    run_benchmark.main([
        "--benchmark", "coding",
        "--baseline", "no_memory",
    ...
    summary = next((tmp_path / "cli").rglob("summary.json"))
    ...
    # Manifest seed vs CLI seed
    manifest = tmp_path / "manifest.json"
    manifest.write_text(json.dumps({"seed": 42, "tasks": []}), encoding="utf-8")
    captured = []
    monkeypatch.setattr(run_benchmark, "load_manifest_tasks", lambda *args, **kwargs: [t])
    monkeypatch.setattr(
        run_benchmark,
        "run_task",
...
def test_scoring_evaluations():
    ...
    score_bad = _score_result("database", evaluator_bad, "task", "result", None)
    assert score_bad["task_score"] == 0.0
    assert score_bad["task_success"] is False

    # Rating agreement
    class BargainEvaluator:
        metrics = {"task_evaluation": {"buyer": {"strategy": 5, "outcome": 4}, "seller": {"strategy": 4, "outcome": 5}}}
        def evaluate_task_world(self, task, result): pass

    env = type("World", (), {"agreement_reached": True})()
    score_bargain = _score_result("bargaining", BargainEvaluator(), "task", "summary", env)
    assert score_bargain["score_status"] == "available"
    assert score_bargain["task_success"] is True
...
def test_task_timeout_records_timeout_status_and_allows_rerun(tmp_path, monkeypatch):
    ...
    assert "task_timeout" in [e["event"] for e in events]
    assert events[-1]["status"] == "timeout"

    # Resume does not skip timed-out tasks
    called = []
    monkeypatch.setattr(
        "marble.experiments.run_benchmark._run_real_episode",
        lambda *args, **kwargs: called.append(True) or {"score_status": "available", "task_score": 1.0},
...
def test_visible_k_retriever_accepted_without_warning(capsys, tmp_path):
    t = _task()
...
def test_default_max_cards_is_five_across_entrypoints():
    ...
    assert step.max_cards == 5
    parser = _build_arg_parser()
    args = parser.parse_args(["--benchmark", "database"])
    assert args.max_cards == 5
    sig = inspect.signature(GovernedMemory.visible_keys)
    assert sig.parameters["top_k"].default == 5


def test_describe_controller(tmp_path):
    from marble.experiments.run_benchmark import describe_controller

    assert describe_controller("ours_rl", controller_checkpoint="runs/ours_rl_policy.json") == "local_policy(linear;ours_rl_policy.json)"
    assert describe_controller("heuristic") == "heuristic"
    assert describe_controller("global_add_all") == "global_add_all"
    assert "adapter=none" in describe_controller("ours_base")

    sft_dir = tmp_path / "qwen_sft"
    sft_dir.mkdir()
    assert "adapter=qwen_sft" in describe_controller("ours_sft", controller_checkpoint=str(sft_dir))
    assert "adapter=qwen_rl" in describe_controller("ours_rl", qwen_api_model="qwen_rl")


def test_controller_validation_and_aliases(tmp_path, monkeypatch):
    # Reject json checkpoint for qwen baselines
    ...
    # Base baseline allows running without checkpoint
    monkeypatch.setattr("marble.controllers.qwen_lora.api_generate_fn", lambda *args, **kwargs: (lambda p: "{\"visibility\": \"absent\"}"))
    assert make_controller("ours_base", qwen_api_base="http://fake", qwen_api_key="fake") is not None

    # Aliases for linear policy
    p = tmp_path / "policy.json"
